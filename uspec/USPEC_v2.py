...
def USPEC(fea, Ks, distance = 'euclidean', p=1000, Knn=5, maxTcutKmIters=100, cntTcutKmReps=3):
    ...
    data = loadmat('repCenter.mat')
    repClsLabel = data['repClsLabel']
    repClsLabel = np.squeeze(repClsLabel,axis=1) - 1
    repClsCenters = data['repClsCenters']
    # print(repClsLabel[0:10])
    # print(repClsCenters[0:10,:])
    centerDist = pdist2_fast(fea, repClsCenters, distance)
    # print(repClsCenters)
    # print("centerDis")
    
    #print(centerDist[0:10,:])
    ...
    for i in range(cntRepCls):
        ...
        #print('distance')
        #print(pdist2_fast(fea[np.where(minCenterIdxs == i)[0], :], RpFea[np.where(repClsLabel == i)[0], :], distance)[0:10,:])
        #print(fea[np.where(minCenterIdxs == i)[0], :])
        tmp = np.argmin(
    ...
    #print(nearestRepInRpFeaIdx[0:10,:])
    # the result is 2 dim
    end = time.time()
...
def TCut_for_bipartite_graph(B, Nseg, maxKmIters=100, cntReps=3):
    ...
    # print("eigen vector:")
    # print(evec[:10, :10])
    idx = (-eval).argsort()[:Nseg]
    Ncut_evec = D.dot(evec[:, idx])
    # print(Nseg)
    print('Ncut_evec:')
    # print(Ncut_evec.shape)
    print(Ncut_evec[:10, :5])
